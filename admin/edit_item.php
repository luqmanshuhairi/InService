...
if(!isset($_GET['id']))
{
    header("location:admin/view_items.php");
}

?>

<div class="formfield" style="float: right; padding-right: 80px;">
<button class="upload" onclick="location.replace('view_items.php')">BACK</button>
<div id="expand">
</div></div>
...
<form action="<?php echo $_SERVER['REQUEST_URI'];?>" id="form" enctype = "multipart/form-data" method = "post">
  <div id="wrapper">
    ...
    <?php
      if(isset($_POST["edit"])) 
      {
        ...
        if($_FILES['bookimg']['size'] != 0 && $_FILES['bookimg']['error'] == 0)
        {
          ...
          {
            echo "<b><p style='color:red'>Unable to upload item image</p></b>";
          }
        }
    ...
    <label for="name">Name of the item <span style="color: red;"><sup>*</sup></span></label></div><br>
    <input type="text" name="book_name" value="<?php echo $val['bname']; ?>" required>
    
    <div class="formfield"><br>
    <label for="author">Shop <span style="color: red;"><sup>*</sup></span></label></div><br>
    <input type="text"  name="author" value="<?php echo $val['author']; ?>" required>
    
    <div class="formfield"><br>
    <label for="keywords" style="margin :auto;">Item Description <span style="color: red;"><sup>*</sup></span></label></div><br>
